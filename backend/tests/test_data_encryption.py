...
def _set_encryption_env(current: bytes, key_id: str, previous: bytes | None = None) -> None:
...
def test_key_rotation_fallback() -> None:
    ...
    _set_encryption_env(old_key, "k-old")
    encrypted_with_old = encrypt_value("legacy-value")
    old_hash = blind_index("legacy-value")

    _set_encryption_env(new_key, "k-new", previous=old_key)
